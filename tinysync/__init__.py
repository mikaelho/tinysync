...
class Handler:
    ...
    def on_change(self, target, changes):
...
def track(
    ...
    * `dot_access`: Optional - True or False to indicate whether you want this 
    ...
    dot notation. Default is True unless changed globally by calling the
    `dot_off` function.
    """
...
def atomic(tracked):
    ...
    Includes normal tracked object context management, i.e. update locking and
    delay of any save or sync. """

    with tracked:
        tracked._tracker.handler.tracked = False
        transient_copy = copy.deepcopy(tracked)
        try:
            yield transient_copy
        except:
            tracked.__subject__ = transient_copy
            tracked._tracker.handler.on_change(tracked, [])
            raise
        else:
            tracked._tracker.handler.tracked = True
            all_changes = list(dictdiffer.diff(transient_copy, tracked))
            tracked._tracker.handler.on_change(tracked, all_changes)
        finally:
            tracked._tracker.handler.tracked = True
